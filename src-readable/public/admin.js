...
// Proprietário/ROOT (acima de admin geral) — controla se o botão de
// conceder/remover ROOT aparece na aba Usuários. Setado em initRootPanel().
...
async function loadUsers() {
  ...
  // Conceder/remover ROOT — só aparece pra quem já é ROOT (ver isRootUser),
  // mas a rota também exige requireRoot no servidor, então mesmo que
  // alguém force o botão a aparecer via devtools, o servidor recusa.
...
// ---------- PROPRIETÁRIO / ROOT + IA COPILOTO ----------
// Só aparece a aba inteira pra quem é ROOT de verdade (acima de admin
// geral). Se ninguém ainda é ROOT e essa conta é admin, mostra só o botão
// de virar a primeira (bootstrap) — sem acesso ao resto até isso acontecer.
async function initRootPanel(me) {
  ...
  try {
    ...
    if (!me.is_root && !status.can_bootstrap) {
      navBtn.remove(); // nem root, nem pode virar o primeiro — não mostra nada
      return;
    }
    ...
    // É ROOT de verdade — carrega o painel inteiro.
    isRootUser = true;
    ...
    loadUsers(); // reconstrói a tabela de Usuários já com o botão de ROOT visível
    loadRootOverview();
